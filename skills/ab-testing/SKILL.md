...
- `docs/tech-stack.md` - `deployment_provider`（存在する場合）

### Step 2: Feature Flag 基盤の選定

`deployment_provider` と運用予算で最適な基盤を選定:

| 条件 | 推奨基盤 | 理由 |
|------|----------|------|
| `deployment_provider=vercel` かつ有料運用 | Vercel Edge Config + Feature Flags | Vercelネイティブで低レイテンシ |
| `deployment_provider=cloudflare-pages` | Cloudflare KV / Workers + custom assignment | Provider内で一貫した配信制御 |
| provider非依存で早く始める | PostHog Feature Flags | 無料枠あり、分析統合 |
...
- [ ] `deployment_provider` と選定した Feature Flag 基盤の整合が取れている
